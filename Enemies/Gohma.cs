...
// create based on keese
public class Gohma : IEnemy, ICollideable

{
    ...
    public Gohma(Vector2 position, bool hasKey)
    {
        ...
        velocity.Normalize();
        velocity *= Constants.GohmaSpeed;
        sprite = EnemySpriteFactory.Instance.CreateGohmaSprite();
        destinationRectangle = new Rectangle((int)position.X, (int)position.Y, Constants.GohmaWidth, Constants.GohmaHeight);
        alive = true;
        hp = Constants.GohmaHp;
        canTakeDamage = true;
        if (hasKey == null)
        {
            keyStatus = false;
    ...
    public void ChangeDirection()
    {
        ...
        {
            case 0:
                velocity = new Vector2(Constants.KeeseSpeed, -Constants.GohmaSpeed);
                break;
            case 1:
                velocity = new Vector2(-Constants.KeeseSpeed, Constants.GohmaSpeed);
                break;
    ...
    public Rectangle getHitbox()
    {
        ...
        if (alive)
        {
            hitbox = new Rectangle((int)position.X, (int)position.Y, Constants.GohmaHitboxWidth, Constants.GohmaHitboxHeight);
        }
